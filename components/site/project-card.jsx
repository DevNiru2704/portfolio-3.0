...
export function ProjectCard({ project, index = 0, featured = false }) {
  ...
  const bg = useMotionTemplate`radial-gradient(220px circle at ${mx}px ${my}px, hsl(${project.accent} / 0.18), transparent 60%)`;
      ...
      >
        ...
        <div className="relative mt-6 flex flex-1 items-end justify-between">
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span className="inline-flex items-center gap-1"><Star className="h-3 w-3" />{project.metrics.stars}</span>
            <span className="inline-flex items-center gap-1"><GitFork className="h-3 w-3" />{project.metrics.forks}</span>
            <span className="inline-flex items-center gap-1"><Users className="h-3 w-3" />{project.metrics.users}</span>
          </div>
