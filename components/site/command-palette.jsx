    ...
    <Ctx.Provider value={value}>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList className="max-h-[420px]">
          <CommandEmpty>No results. Try ‘projects’, ‘contact’ or ‘dark’.</CommandEmpty>
          ...
          <CommandGroup heading="Actions">
            <Item
              ...
              label="Open CMS Preview"
              hint="public demo"
              onSelect={() => go('/cms-preview')}
            />
          ...
          <CommandGroup heading="Social">
            <Item
              icon={Github}
              label="GitHub — @nirmalya"
              onSelect={() => { window.open(owner.github, '_blank'); setOpen(false); }}
            />
            <Item
              icon={Linkedin}
              label="LinkedIn"
              onSelect={() => { window.open(owner.linkedin, '_blank'); setOpen(false); }}
            />
            <Item
              icon={Mail}
              label={`Email · ${owner.email}`}
              onSelect={() => { window.location.href = `mailto:${owner.email}`; setOpen(false); }}
            />
          </CommandGroup>
        </CommandList>
      </CommandDialog>
