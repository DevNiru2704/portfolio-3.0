...
function ProjectsPage() {
  const categories = useMemo(() => ['All', ...Array.from(new Set(seedProjects.map((p) => p.category)))], []);
  const allTags = useMemo(
    () => Array.from(new Set(seedProjects.flatMap((p) => p.tags))).sort(),
    [],
  );
  ...
  const filtered = useMemo(() => {
    let list = seedProjects.filter((p) => (cat === 'All' || p.category === cat));
    ...
    if (sort === 'Stars') list = [...list].sort((a, b) => b.metrics.stars - a.metrics.stars);
    ...
    <div className="relative">
      ...
      <section className="container py-12">
        <div className="mb-4 flex items-center justify-between font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
          <span>{filtered.length} project{filtered.length === 1 ? '' : 's'} · filtered</span>
