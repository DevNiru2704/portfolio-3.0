import { seedPhilosophy } from '@/config/seed';
          ...
          {seedPhilosophy.map((p, i) => (
            <li key={p.title} className="group relative overflow-hidden rounded-2xl border border-border bg-card/60 p-6">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-xs text-muted-foreground/50 tabular-nums">{String(i + 1).padStart(2, '0')}</span>
              </div>
