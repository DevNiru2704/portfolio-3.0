...
import { seedProjects, seedExperience, seedLab, seedTestimonials, seedSystemStatus } from '@/config/seed';
...
function Home() {
  ...
  const featured = seedProjects.filter((p) => p.featured);
            ...
            <div className="relative flex items-start justify-between">
              <div>
                ...
                <h3 className="mt-4 text-2xl font-semibold tracking-tight md:text-3xl">
                  The dashboard that runs this site.
                </h3>
                <p className="mt-2 max-w-md text-sm text-muted-foreground">
                  Live, public, read-only walkthrough of the real CMS — analytics, project manager, markdown editor, deployment logs.
                </p>
              </div>
            ...
            <div className="relative mt-8 grid grid-cols-3 gap-3 text-xs">
              {['Projects', 'Posts', 'Messages'].map((l, i) => (
                <div key={l} className="rounded-xl border border-border bg-background p-3">
                  ...
                  <div className="mt-1 text-xl font-semibold tabular-nums text-gradient-accent">
                    {[12, 27, 184][i]}
                  </div>
                  <div className="mt-2 h-1 w-full overflow-hidden rounded-full bg-secondary">
                    <div
                      className="h-full rounded-full bg-[hsl(var(--glow-cyan))]"
                      style={{ width: `${[60, 80, 45][i]}%` }}
                    />
                  </div>
                </div>
              ))}
                  ...
                  <span className="flex items-center gap-2 truncate">
                    <span
                      className={`h-1.5 w-1.5 rounded-full ${
                        l.status === 'Done' ? 'bg-emerald-400' : 'bg-amber-400 animate-pulse-dot'
                      }`}
                    />
      ...
      <section className="container py-20">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {seedTestimonials.map((t, i) => (
            ...
            >
              <div className="absolute -right-6 -top-6 text-7xl font-bold text-muted/30">”</div>
              <blockquote className="relative text-sm leading-relaxed text-foreground/90">“{t.quote}”</blockquote>
              <figcaption className="relative mt-4 flex items-center gap-3">
                <span
                  className="grid h-9 w-9 place-items-center rounded-full border border-border bg-background text-xs font-semibold"
                  aria-hidden
                >
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1.4fr_1fr] lg:items-center">
            <div>
              ...
              <p className="mt-3 max-w-xl text-muted-foreground">
                Currently available for freelance and full-time roles. Based in {owner.location} · working with teams worldwide.
              </p>
            ...
            <div className="grid gap-3">
              {[
                ...
                { icon: Cpu, label: 'Active projects', value: seedSystemStatus.metrics.activeProjects },
                { icon: Zap, label: 'P95 latency', value: seedSystemStatus.metrics.p95Latency },
              ].map((m) => (
...
function CommandPreview({ onOpen }) {
  return (
    <button
      onClick={onOpen}
